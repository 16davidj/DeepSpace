...
public class OVRPlayerController1 : MonoBehaviour
{
	...
	///just a vector for normals
	Vector3 normalVector;
	bool slopeGood = false;
	/// <summary>
	...
	public float slopeLim;
	//bool sprint = false;
	void OnControllerColliderHit(ControllerColliderHit hit) {

		normalVector = hit.normal;
		if (Mathf.Abs (normalVector.x) >= slopeLim || Mathf.Abs (normalVector.z) >= slopeLim) {
			slopeGood = false;
		} else {
			slopeGood = true;
		}
	...
	public virtual void UpdateMovement()
	{
		...
		bool moveJump = Input.GetKey (KeyCode.Space) || Input.GetMouseButton(1);
		...
		if(moveJump && Controller.isGrounded && slopeGood == true)
			MoveThrottle += ort * (transform.lossyScale.y  * Vector3.up);
